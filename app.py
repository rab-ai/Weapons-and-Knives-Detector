...
def get_conf_threshold(form, key, default=0.2):
    try:
...
def detect_image():
    conf_threshold = request.form.get('conf_threshold', '0.2')  # Default to '0.2' if not provided
    try:
        conf_threshold = float(conf_threshold)
    except ValueError:
        conf_threshold = 0.2  # Fallback to default if conversion fails
    ...
    if file:
        ...
        file.save(filepath)
        original_path, result_path, detected_weapons, ai_response = detect_objects_in_photo(filepath, conf_threshold)
...
def detect_video():
    ...
    if file:
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
        file.save(filepath)
        result_video_path = detect_objects_in_video(filepath, conf_threshold)
...
def start_webcam():
    global capturing
    capturing = True
    conf_threshold = request.args.get('conf_threshold', '0.2')  # Default to '0.2' if not provided
    try:
        conf_threshold = float(conf_threshold)
    except ValueError:
        conf_threshold = 0.2  # Fallback to default if conversion fails
    return redirect(url_for('webcam', conf_threshold=conf_threshold))
...
def gen_frames(conf_threshold):
    ...
    while capturing:
        ...
        if frame_count % process_every_n_frames == 0:
            frame = process_frame(frame, yolo_model, conf_threshold)
...
def video_feed():
    conf_threshold = float(request.args.get('conf_threshold', 0.2))  # Default to 0.2 if not provided
    return Response(gen_frames(conf_threshold), mimetype='multipart/x-mixed-replace; boundary=frame')
